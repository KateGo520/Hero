...
# from findbug_oneoldcheck_up import check_repo_fo
# from one_check_api.findbug_oneoldcheck_up import check_repo_fo
...
# 获取删除的repo的list
def get_vendor_mod_list(host, user, password, db_name):
    n_name_list = []
    sql = "SELECT repo_name FROM vendor_mod"
    try:
        # 执行sql语句
        ...
        print("get vendor_repo list error:", exp, '************************************************************')
        return []
...
def check_bug_old_repo(modname, name, semantic, bug_type_num, bug_list, break_type_num, break_list, headers,
    ...
    # 获取上游repos
    (imp_final_siv_list, imp_final_list, insert_e, search_e, check_e, update_e, insert_s, update_s, issue,
     vendor_list) = get_up_repo(import_list, dir_name_list, vendor_list, fullname, host, user, password, db_name,
    ...
    # 获取vendor-mod列表
    if imp_final_list:
        vendor_mod_list = get_vendor_mod_list(host, user, password, db_name)
    for imp in imp_final_list:
        ...
        if check_type == 0:
            ...
            if search_r == 2:
                for r in r_list:
                    if r == '1-1':
                        if imp_mod_name not in vendor_mod_list:
                            # 问题1-1，A，旧用户升级预警
                            bug_type_num[1] = bug_type_num[1] + 1
                            if imp_mod_name in vendor_list:
                                bug_list[1] = bug_list[1] + '^1.1:' + imp_mod_name + ' ' + r_vname
                            else:
                                bug_list[1] = bug_list[1] + '$1.1:' + imp_mod_name + ' ' + r_vname
                            print('db get bug 1-1:', imp + ' ' + r_vname)
                            # 获取主版本号：
                            r_vname_v = re.findall(r"^v(\d+?)\.", r_vname)
                            if r_vname_v:
                                r_vname_v_siv_s = str(r_vname_v[0])

                            else:
                                if re.findall(r"^v(\d+?)$", r_vname):
                                    r_vname_v = re.findall(r"^v(\d+?)$", r_vname)
                                    r_vname_v_siv_s = str(r_vname_v[0])
                                else:
                                    r_vname_v_siv_s = '-1'
                            r_vname_v_siv_i = int(r_vname_v_siv_s)
                            if r_vname_v_siv_i >= 2:
                                # 问题2-0，B，下游新用户升级预警
                                bug_type_num[2] = bug_type_num[2] + 1
                                if imp_mod_name in vendor_list:
...
def search_check_go_repos(fullname, host, user, password, db_name):
    ...
    sql = "SELECT v_name,num1_0,num1_1,break1,num2_0,num2_1,break2,num3_0,num3_1,break3,num4_0,num4_1,break4,v_hash " \
          "FROM check_go_repos_20200529 WHERE full_name='%s' AND go_mod=2" % fullname
    try:
        # 执行sql语句
        db_check = pymysql.connect(host, user, password, db_name)
        check_cursor = db_check.cursor()
        ...
        if check_result:
            if check_result[0][0]:
                v_name = check_result[0][0]
            elif check_result[0][10]:
                v_name = check_result[0][13]
            else:
                v_name = ''
            # 有该项
            ...
            if check_result[0][10] > 0 or check_result[0][11] > 0 or check_result[0][12] > 0:
                # 有影响,问题四
                prob_list.append('4-0')
            if prob_list:
                print('check_go_repos_20200529 有该项，有影响', fullname)
                return 2, prob_list, v_name
            else:
                print('check_go_repos_20200529 有该项，无影响', fullname)
                return 1, prob_list, v_name
        else:
            # 无该项
            print('check_go_repos_20200529 无该项')
            sql = "SELECT v_name,num1_0,num1_1,break1,num2_0,num2_1,break2,num3_0,num3_1,break3,num4_0," \
                  "num4_1,break4,v_hash FROM check_go_repos WHERE full_name='%s' AND go_mod=2" % fullname
            try:
                # 执行sql语句
                db_check = pymysql.connect(host, user, password, db_name)
                check_cursor = db_check.cursor()
                ...
                if check_result:
                    ...
                    if check_result[0][10] > 0 or check_result[0][11] > 0 or check_result[0][12] > 0:
                        ...
                        prob_list.append('4-0')
                    if prob_list:
                        print('check_go_repos 有该项，有影响', fullname)
                        return 2, prob_list, v_name
                    else:
                        print('check_go_repos 有该项，无影响', fullname)
                        return 1, prob_list, v_name
                else:
                    # 无该项
                    print('check_go_repos 无该项', fullname)
                    return 0, prob_list, ''
            except Exception as exp:
                print("check check_go_repos ", fullname, " error:", exp, '****************'
                                                                         '********************************')
                return -1, [], ''
    except Exception as exp:
        print("check check_go_repos_20200529 ", fullname, " error:", exp, '*************************************'
                                                                          '***********')
        return -1, [], ''
...
def main():
    ...
    # fullname_l = ['tmrts/go-patterns', 'Netflix/chaosmonkey', 'golang/groupcache']
    # page 11[0]  8
    (r, r_list) = get_repo_name_list(host, user, password, db_name)
    if r == 1:
        for fullname in r_list:
            # 从数据库获得fullname
            check_date = int(time.strftime('%Y%m%d', time.localtime(time.time())))
            (insert_error, search_error, check_error, update_error, insert_success,
             update_success, issue_l) = update_old_bug(fullname, check_date, host, user, password, db_name,
                                                       insert_error, search_error, check_error, update_error,
                                                       insert_success, update_success, issue_l, time_w)

    print(search_error, insert_error, check_error, update_error)
