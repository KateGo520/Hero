...
from check_200.final_file.find_local_use import check_file_detail
...
# 从数据库读取某一repo的下游列表
def get_down_list(repo_name, host, user, password, db_name):
    # 查询该存储库是否存在于数据库中，返回查询数量
...
def main():
    ...
    (r_down, down_list) = get_down_list(fullname, host, user, password, db_name)
    if r_down == 0:
        print('暂时没有找到下游repos')
    elif r_down == -1:
        print('读取repo_depend失败')
